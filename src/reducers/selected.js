  ...
  switch (action.type) {
    case "ADD_SELECTED":
      ...
      const rows = action.rows
        .filter((el) => !rowIds.includes(el.rowId))
        .map((el) => ({
          name: el.name,
          rowId: el.rowId,
          entryPrice: 0,
    ...
    case "RECALCULATE_MIN_STOPLOSS":
      return state.map((asset) => {
        ...
        const revisedPrice =
          asset.stopLossPrice > defaultPrice
            ? defaultPrice
            : asset.stopLossPrice;
        return {
          ...asset,
      ...
      if (action.defaultStop) {
        return {
          ...
          stopLossPrice: Math.min(
            ...
            roundToTwo(
              asset.entryPrice * ((100 - parseFloat(action.defaultStop)) * 0.01)
            )
          ),
        };
      } else {
        return {
          ...asset,
          targetPrice: roundToTwo(
            asset.entryPrice * ((100 + parseFloat(action.defaultTake)) * 0.01)
          ),
        };
      }
    } else {
...
const calcAllocatedFunds = (state, action) => {
  const alreadyAllocated = state
    .filter((asset) => asset.rowId !== action.rowId)
    .reduce((acc, cur) => acc + cur.allocatedFunds, 0);
      ...
      return {
        ...
        allocatedFunds:
          action.funds === 100
            ? 0
            : Number(
                Math.max(0, asset.allocatedFunds * changePerAsset).toFixed(1)
              ),
      };
    }
  });
